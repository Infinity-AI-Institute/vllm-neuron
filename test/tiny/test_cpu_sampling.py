...
def test_hardware_vs_cpu_sampling_comparison():
    ...
    deterministic_configs = [
        ...
        ("greedy_with_seed", SamplingParams(temperature=0.0, max_tokens=15, seed=123)),
        (
    ...
    non_deterministic_configs = [
        (
            "nucleus_sampling",
            SamplingParams(temperature=0.7, max_tokens=10, top_p=0.95),
        ),
        (
            "high_temperature",
            SamplingParams(temperature=0.8, max_tokens=10, top_k=50, top_p=0.9),
        ),
        ("pure_top_k", SamplingParams(temperature=1.0, max_tokens=15, top_k=20)),
        ("pure_top_p", SamplingParams(temperature=1.0, max_tokens=15, top_p=0.8)),
        (
            "combined_top_k_top_p",
            SamplingParams(temperature=0.6, max_tokens=12, top_k=40, top_p=0.85),
        ),
        (
            "repetition_penalty",
            SamplingParams(temperature=0.7, max_tokens=25, repetition_penalty=1.1),
        ),
        (
            "frequency_penalty",
            SamplingParams(temperature=0.6, max_tokens=20, frequency_penalty=0.5),
        ),
        (
            "presence_penalty",
            SamplingParams(temperature=0.7, max_tokens=18, presence_penalty=0.3),
        ),
        (
            "combined_penalties",
            SamplingParams(
        ...
        ("min_p_sampling", SamplingParams(temperature=0.7, max_tokens=12, min_p=0.05)),
        (
            "stop_sequences",
            SamplingParams(temperature=0.8, max_tokens=50, stop=["\n", ".", "END"]),
        ),
        (
            "logprobs_request",
            SamplingParams(temperature=0.8, max_tokens=12, logprobs=5),
        ),
        (
            "seed_deterministic_high_temp",
            SamplingParams(temperature=0.8, max_tokens=20, seed=42),
        ),
        ("extreme_low_temp", SamplingParams(temperature=0.01, max_tokens=10, top_k=5)),
        (
            "extreme_high_temp",
            SamplingParams(temperature=2.0, max_tokens=15, top_p=0.95),
        ),
        (
            "complex_combination",
            SamplingParams(
    ...
    if hw_loaded and cpu_loaded:
        ...
        for config_name, sampling_params in deterministic_configs:
            ...
            # Compare outputs
            matches = 0
            for i, (hw_out, cpu_out) in enumerate(zip(hw_outputs, cpu_outputs)):
            ...
            print(f"   📊 Match Rate: {matches}/{len(prompts)} ({match_rate:.1f}%)")
            print(f"   ⏱️  Timing: HW={hw_time:.3f}s, CPU={cpu_time:.3f}s")

            # For deterministic cases, we expect high match rates
            if match_rate < 80:
                print("   ⚠️  WARNING: Low match rate for deterministic sampling!")

        overall_match_rate = (
            (total_matches / total_comparisons * 100) if total_comparisons > 0 else 0
        )
        print(
        ...
        if hw_loaded:
            try:
                ...
                hw_outputs = hw_engine.generate(
                    prompts[:3], sampling_params
                )  # Use fewer prompts for speed
                hw_time = time.time() - start_time
                hw_success = True
                hw_error = None
                hw_successes += 1
        ...
        if cpu_loaded:
            try:
                ...
                if config_name == "logprobs_request":
                    assert cpu_outputs[0].outputs[0].logprobs, (
                        "No logprobs when logprobs were requested"
                    )
                cpu_success = True
                cpu_error = None
                cpu_successes += 1
    ...
    if hw_loaded:
        hw_success_rate = (hw_successes / total_tests * 100) if total_tests > 0 else 0
        print(
            f"🔧 Hardware sampling success rate: {hw_success_rate:.1f}% ({hw_successes}/{total_tests})"
        )

    if cpu_loaded:
        cpu_success_rate = (cpu_successes / total_tests * 100) if total_tests > 0 else 0
        print(
