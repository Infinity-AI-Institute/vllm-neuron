...
import logging
import os
from test.e2e.online.online_server_runner import OnlineCfg, run_online_integration
...
def check_server_logs_for_error(seq_len, tp_degree, log_file="vllm_server_out.txt"):
    """
...
def verify_sequence_parallel_failure(title, seq_len, tp_degree, exception=None):
    """
...
def test_sequence_parallel(sequence_parallel_enabled, expect_failure):
    ...
    if expect_failure:
        logger.info(f"[{title}] Expecting server to fail with validation error...")

        try:
            run_online_integration(cfg)
        except Exception as e:
            verify_sequence_parallel_failure(title, SEQ_LEN, TP_DEGREE, exception=e)
        else:
            verify_sequence_parallel_failure(title, SEQ_LEN, TP_DEGREE, exception=None)
    else:
...
if __name__ == "__main__":
    ...
    test_sequence_parallel(sequence_parallel_enabled=False, expect_failure=False)
