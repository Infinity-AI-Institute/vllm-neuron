...
def resolve_model_dir(
    logical: str, require_fsx: bool = False, use_fsx_only: bool = False
) -> Tuple[str, bool]:
    """
    ...
    if not fsx_path.exists():
        if require_fsx:
            ...
            raise FileNotFoundError(
                f"Model not found on FSx: {fsx_path} (original: {original_logical})"
            )
        logger.info(f"Model not found on FSx, returning as model ID: '{logical}'")
        return logical, False
    ...
    if use_fsx_only:
        logger.info(f"Returning FSx path directly (use_fsx_only=True): '{fsx_path}'")
    ...
    if not ssd_root_path.exists() or not os.access(ssd_root, os.W_OK):
        logger.warning(f"SSD not writable ({ssd_root}), using FSx path: '{fsx_path}'")
        return str(fsx_path), False
    ...
    try:
        ...
        shutil.copytree(fsx_path, dst, dirs_exist_ok=False, copy_function=shutil.copy2)
