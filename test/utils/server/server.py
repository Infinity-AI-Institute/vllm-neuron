...
LARGE_LLM_LIST = ["dbrx", "llama-3.3-70B", "llama-3.1-405b", "qwen-3-235b", "llama-4"]
...
def _get_modules_to_not_quantize(
    local_model_dir: str, override_json_path: Optional[str] = None
...
class VllmServer:
    ...
    def __init__(
        ...
        tp_degree: int = 24,
        dp_degree: int = 1,
        dp_rank: int = -1,  # -1 means not specified
        ...
        **kwargs,
        ...
        self.lora_modules = {}
        self.lora_modules_cpu = {}
        self.max_loras = max_loras
        self.max_cpu_loras = max_cpu_loras
        self.max_lora_rank = max_lora_rank
        self.reasoning_parser = kwargs.get("reasoning_parser", None)

        # Allow cores to be overridden for parallel execution with core isolation
        self.cores = kwargs.get("cores", f"0-{self.tp_degree * self.dp_degree - 1}")
        ...
        if self.enable_lora:
            self._download_lora_adapters(lora_ckpt_dict)

        self.enable_auto_tool_choice = kwargs.get("enable_auto_tool_choice", False)
    ...
    def _health_probe(self, url: str, retries: int, delay: int, process=None) -> bool:
        for _ in range(retries):
            # If the child died at any point, fail fast
            if process is not None and process.poll() is not None:
                raise RuntimeError(f"vLLM engine exited with code {process.returncode}")
    ...
    def _retries_for_model(self) -> int:
        ...
        return (
            480 if any(s in key for s in (s.lower() for s in LARGE_LLM_LIST)) else 120
        )

    def _download_lora_adapters(self, lora_ckpt_dict):
        if self.max_cpu_loras > 0:
            os.environ["VLLM_ALLOW_RUNTIME_LORA_UPDATING"] = "1"
        lora_adapter_manager = LoRAAdapterManager(lora_ckpt_dict)
        self.override_neuron_config["lora_ckpt_json"] = (
            lora_adapter_manager.get_lora_json_file()
        )

    def _extract_lora_modules(self) -> str:
        lora_ckpt_json = self.override_neuron_config.get("lora_ckpt_json", None)
        if lora_ckpt_json:
            with open(lora_ckpt_json, "r") as f:
                content = json.load(f)
                lora_ckpt_dir = content.get("lora-ckpt-dir", os.getcwd())
                lora_ckpt_paths = content["lora-ckpt-paths"]
                lora_ckpt_paths_cpu = content.get("lora-ckpt-paths-cpu", {})

            for adapter_id, path in lora_ckpt_paths.items():
                self.lora_modules[adapter_id] = os.path.join(lora_ckpt_dir, path)

            for adapter_id, path in lora_ckpt_paths_cpu.items():
                self.lora_modules[adapter_id] = os.path.join(lora_ckpt_dir, path)
                self.lora_modules_cpu[adapter_id] = os.path.join(lora_ckpt_dir, path)
        return " ".join(f"{key}={value}" for key, value in self.lora_modules.items())
    ...
    def run_inference_demo(self, resolved_model_dir: str) -> None:
        ...
        try:
            ...
            if self.compiled_model_path:
                assert os.path.isdir(self.compiled_model_path) and os.listdir(
                    self.compiled_model_path
                ), f"No compile artifacts found in {self.compiled_model_path}"
            print(f"Inference demo completed successfully. rc={result.returncode}")
        except subprocess.CalledProcessError as e:
            print(f"Inference demo failed with rc={e.returncode}")
            raise

    def start_vllm_server(self, server_log_name=None):
        health_ok = False
        SERVER_LOG = (
            "vllm_server_out.txt" if server_log_name is None else server_log_name
        )

        # resolve model dir from FSX → SSD (if SSD writable) using your utils
        resolved_model_dir, used_ssd = resolve_model_dir(self.logical_model_path)
        self.served_model_id = resolved_model_dir
        if used_ssd:
            print(
                f"[vLLM] Using model dir: {resolved_model_dir} (Model checkpoint copied from SSD)"
            )
        else:
            print(
                f"[vLLM] Using model dir: {resolved_model_dir} (Model checkpoint loaded from HF)"
            )
        ...
        try:
            ...
            start_script = os.path.join(os.path.dirname(__file__), "start_server.sh")
            if not os.path.exists(start_script):
                raise FileNotFoundError(f"start_server.sh not found at {start_script}")
            ...
            if self.draft_model_path is not None:
                resolved_draft_dir, _ = resolve_model_dir(self.draft_model_path)
                resolved_draft_dir = resolved_draft_dir.rstrip("/")

                spec_cfg = {
                    "model": resolved_draft_dir,
                    "num_speculative_tokens": self.speculation_len
                    if self.speculation_len is not None
                    else 4,
            ...
            if self.is_quantized_checkpoint:
                ...
                if self.quantization_dtype:
                    self.override_neuron_config["quantization_dtype"] = (
                        self.quantization_dtype
                    )
                if self.quantization_type:
                    self.override_neuron_config["quantization_type"] = (
            ...
            if self.enable_chunked_prefill:
                ...
                if (self.num_blocks_override or 0) > 0:
                    args += ["--num-gpu-blocks-override", str(self.num_blocks_override)]
            else:
            ...
            if self.enable_lora:
                args += [
                    ...
                    str(self.max_loras),
                    "--max-cpu-loras",
                    str(self.max_cpu_loras),
                    "--max-lora-rank",
                    str(self.max_lora_rank),
                    "--lora-modules",
                    self._extract_lora_modules(),
                ]

            # -------- Combined Additional Config --------
            # Combine override_neuron_config and additional_config into a single additional_config JSON
            combined_additional_config = {}

            # Add additional_config items at the top level
            if self.additional_config:
                combined_additional_config.update(self.additional_config)

            # Add override_neuron_config as a nested key
            if self.override_neuron_config:
                combined_additional_config["override_neuron_config"] = (
                    self.override_neuron_config
                )

            # Pass the combined config if we have anything to pass
            if combined_additional_config:
                args += ["--additional-config", json.dumps(combined_additional_config)]
            ...
            with open(SERVER_LOG, "w") as outfile:
                process = subprocess.Popen(
                    args, text=True, stdout=outfile, stderr=outfile
                )

            # give server some time to start
            time.sleep(60)
            if process.poll() is not None:
                raise RuntimeError(f"subprocess return code: {process.returncode}")

            health_ok = self._health_probe(
    ...
    def start(self, server_log_name=None):
        """
        ...
        port, process, ok = self.start_vllm_server(server_log_name)
        if not ok:
