...
while [[ "${#}" -gt 0 ]]; do
  case "$1" in
    ...
    --additional-config) additional_config="$2"; shift 2 ;;
...
# Only add --data-parallel-rank if explicitly provided (not default -1)
if [[ "${dp_rank}" != "-1" ]]; then
  cmd_args+=(--data-parallel-rank "$dp_rank")
fi
...
# Set NEURON_RT_VISIBLE_CORES if cores parameter is provided
if [[ -n "${cores}" && "${cores}" != "0-31" ]]; then
  export NEURON_RT_VISIBLE_CORES="${cores}"
  echo "Setting NEURON_RT_VISIBLE_CORES=${cores}"
fi
