...
def get_vllm_mm_model_inputs(prompt, single_image, sampling_params, model_name):
    """
    ...
    if "pixtral" in model_name.lower():
        instruct_prompt = instruct_prompt.replace("|image|", "[IMG]")
...
def print_outputs(outputs):
    for output in outputs:
        ...
        logger.info(f"\n[Prompt]\n{prompt!r}\n[Generated]\n{generated_text!r}\n")
        assert generated_text.strip(), f"Output was empty for prompt: {prompt!r}"
