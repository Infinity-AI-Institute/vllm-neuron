...
def _find_lm_head_from_single_safetensors(target_dir: Path) -> Tuple[torch.Tensor, str]:
...
def inject_lm_head_into_draft(
    draft_st_path: Path, lm_head: torch.Tensor
...
def fix_eagle_draft_for_nxdi(
    target_dir: str | Path, draft_dir: str | Path
) -> FixResult:
    """
    Idempotent fixer:
    ...
    if not ready_before:
        ...
        shape, dtype = inject_lm_head_into_draft(draft_dir / SAFETENSORS, lm_head)
        injected = True
...
def prepare_draft_model(
    target_model: str,
    draft_model: str,
    speculation_type: str = "eagle",
) -> Tuple[str, str]:
    """
    Prepare draft model for use with vLLM Neuron.

    Downloads models from HuggingFace if needed and applies necessary conversions
    for Neuron compatibility.

    Args:
        target_model: Target model ID or path
        draft_model: Draft model ID or path
        speculation_type: Type of speculation ("eagle", etc.)

    Returns:
        Tuple of (target_model_path, draft_model_path) ready for use
    """
    target_model_dir, _ = resolve_model_dir(target_model)
    draft_model_dir, _ = resolve_model_dir(draft_model)
    ...
    if not os.path.isabs(draft_model_dir):
        ...
        target_model_dir = snapshot_download(target_model)
        draft_model_dir = snapshot_download(draft_model)

        logger.info("Downloaded target model to: %s", target_model_dir)
        logger.info("Downloaded draft model to: %s", draft_model_dir)

    # Apply speculation-type-specific conversions
    if speculation_type == "eagle":
        logger.info("Converting EAGLE draft model for Neuron compatibility...")
        result = fix_eagle_draft_for_nxdi(target_model_dir, draft_model_dir)
        logger.info("EAGLE draft model conversion complete: %s", result)
    else:
        logger.warning(
            "No conversion implemented for speculation_type=%s", speculation_type
        )

    return target_model_dir, draft_model_dir
...
def _main():
    ...
    ap.add_argument("--target", required=True, help="Path to TARGET model folder")
    ap.add_argument("--draft", required=True, help="Path to EAGLE DRAFT model folder")
    args = ap.parse_args()
