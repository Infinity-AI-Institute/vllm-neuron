...
import logging
from test.e2e.online.configs import (
    EAGLE_CONFIGS,
    INTEGRATION_CONFIGS,
    QWEN_CONFIG,
    MULTI_LORA_CONFIGS,
...
@pytest.mark.parametrize(
    "cfg", INTEGRATION_CONFIGS, ids=[c.name for c in INTEGRATION_CONFIGS]
)
...
def test_qwen():
    ...
    logger.info(
        ...
        QWEN_CONFIG.model,
        QWEN_CONFIG.tp_degree,
    )
    run_online_integration(QWEN_CONFIG)


@pytest.mark.timeout(900)
@pytest.mark.parametrize("cfg", EAGLE_CONFIGS, ids=[c.name for c in EAGLE_CONFIGS])
...
@pytest.mark.parametrize(
    "cfg", TOOL_CALLING_CONFIGS, ids=[c.name for c in TOOL_CALLING_CONFIGS]
)
...
def test_online_multi_lora(cfg):
    """Multi-LoRA online inference integration test"""
    logger.info("Running multi-lora integration test with config: %s", cfg.name)
    run_online_integration(cfg)
