...
MULTI_LORA_CONFIGS = [
    ...
    OnlineCfg(
        name="tinyllama-multi-lora-static",
        model="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
        tp_degree=32,
        batch_size=2,
        max_model_len=128,
    ...
    OnlineCfg(
        name="tinyllama-multi-lora-dynamic",
        model="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
        tp_degree=32,
        batch_size=2,
        max_model_len=128,
...
QWEN_CONFIG = OnlineCfg(
    ...
    max_tokens=50,
    port=8000,
    accuracy_check=False,  # Focus on testing that server starts without validation error
)
...
TOOL_CALLING_CONFIGS = [
    OnlineCfg(
        ...
        custom_chat_template_path=str(
            TEST_DIR / "utils" / "server" / "tool_chat_template_llama3.1_json.jinja"
        ),
        use_chat_url=True,
