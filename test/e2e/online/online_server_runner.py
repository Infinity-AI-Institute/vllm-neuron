...
class OnlineCfg:
    ...
    model: str
    tp_degree: int = 32
    dp_degree: Optional[int] = 1
    dp_rank: Optional[int] = -1  # -1 means not specified
    batch_size: int = 4
...
def _post_with_retries(
    ...
    for _ in range(tries):
        try:
            ...
            last_exc = e
            logger.warning("POST %s failed with %r; retrying in %ss", url, e, sleep_s)
        time.sleep(sleep_s)
    if last_exc:
        raise last_exc
...
def _run_hf_fulltext_prefix_check(
    ...
    prompts: List[str],
    max_model_len: int,
    actual_full_texts: List[str],  # FULL strings: prompt + server continuation
) -> None:
    if not _HF_OK:
        logger.warning(
    ...
    logger.info(
        "Running HF full-text prefix check with model at: %s", served_model_fs_path
    )

    tok = AutoTokenizer.from_pretrained(served_model_fs_path, padding_side="left")
    if (
    ...
    for i, (actual, expected) in enumerate(zip(actual_full_texts, expected_full_texts)):
        if not expected.startswith(actual):
            ...
            raise AssertionError(
                f"HF prefix mismatch at idx={i}:\n"
                f"  actual:   {actual!r}\n"
                f"  expected: {expected!r}"
            )


def start_server(cfg: OnlineCfg, server_log_name: str = None):
    # Handle draft model conversion if needed
    model_path = cfg.model
    draft_model_path = cfg.draft_model_path

    if draft_model_path and cfg.speculation_type:
        from test.utils.eagle_nxdi_util import prepare_draft_model

        # Prepare models (download and convert if needed)
        model_path, draft_model_path = prepare_draft_model(
            cfg.model, draft_model_path, speculation_type=cfg.speculation_type
        )
    ...
    server = VllmServer(
        ...
        lora_ckpt_dict=cfg.lora_ckpt_dict,
        # CP/APC
        ...
        # DP
        **({"cores": cfg.cores} if cfg.cores is not None else {}),
    )

    port, proc, healthy = server.start(server_log_name)
    return server, port, proc, healthy


def send_requests_to_server(cfg, server, port, proc, healthy):
    ...
    if cfg.use_chat_url:
        base_prompts: List[str] = [
            {
                "role": "user",
                "content": "I am going to count from 1 to 250 only once. I will not say anything further. 1,2,3,4,",
            },
    ...
    if cfg.batch_size <= len(base_prompts):
        ...
        reps = (cfg.batch_size + len(base_prompts) - 1) // len(base_prompts)
    ...
    if cfg.tool_call_parser:
        ...
        print(f"prompts={prompts}, type(prompts)={type(prompts)}")

    served_model = getattr(server, "served_model_id", cfg.model)
    if cfg.enable_lora and server:
        # ------------- pick one of the loaded LoRA modules -------------
        served_model = (
            list(server.lora_modules.keys())[0]
            if cfg.max_cpu_loras == 0
            else list(server.lora_modules_cpu.keys())[0]
        )
        memory = "HBM" if cfg.max_cpu_loras == 0 else "CPU"
        logger.info(
            f"Using LoRA model: {served_model} on ${memory} for testing and its checkpoint path is {server.lora_modules[served_model]}"
        )

    # ---- context-safe clamping of max_tokens ----
    try:
        if _HF_OK:
            tok = AutoTokenizer.from_pretrained(served_model, padding_side="left")
            if (
                getattr(tok, "pad_token_id", None) is None
                and getattr(tok, "eos_token_id", None) is not None
            ):
    ...
    if cfg.tool_call_parser:
        payload.update({"messages": messages, "tools": tools, "tool_choice": "auto"})
    ...
    for i, choice in enumerate(data["choices"]):
        ...
        if cfg.reasoning_parser:
            ...
            print(f"reasoning_content[{i}]: {reasoning_content}")
        actual_full_texts.append(str(prompts[i]) + cont)

    # Type sanity check
    for c in data["choices"]:
        assert isinstance(c.get("text", ""), str), "choice.text must be a string"

    # Optional accuracy check (HF) with per-test override
    env_default = os.environ.get("ACCURACY_CHECK", "1").strip().lower() not in (
        "0",
        "false",
        "no",
        "off",
    ...
    do_acc = env_default if cfg.accuracy_check is None else bool(cfg.accuracy_check)
    ...
    if do_acc:
        ...
        logger.info(
            ...
            "config" if cfg.accuracy_check is not None else "env ACCURACY_CHECK",
        )
...
def run_online_integration(cfg: OnlineCfg) -> None:
    ...
    server, port, proc, healthy = start_server(cfg)

    try:
        send_requests_to_server(cfg, server, port, proc, healthy)
    finally:
        try:
