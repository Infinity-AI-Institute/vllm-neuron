...
import logging
import os
from test.e2e.offline.offline_integration_test import (
    vllm_integ_test,
    vllm_integ_test_llama4,
    vllm_integ_test_pixtral,
)
...
PIXTRAL_CONFIG_TRN2 = {
    "title": "pixtral_large_bs1_bs4",
    "model_name_or_path": "mistralai/Pixtral-Large-Instruct-2411",
    "tp_degree": 64,
    "batch_size": 4,
    "max_model_len": 10240,
    "block_size": 10240,
    ...
    "override_neuron_config": {
        "text_neuron_config": {
            ...
            "context_encoding_buckets": [2 * 1024, 4 * 1024, 10 * 1024],
            "token_generation_buckets": [2 * 1024, 4 * 1024, 10 * 1024],
            "skip_warmup": True,
        ...
        "vision_neuron_config": {
            ...
            "seq_len": 10240,
            "torch_dtype": torch.float16,
            "buckets": [2 * 1024, 4 * 1024, 6 * 1024, 8 * 1024, 10 * 1024],
...
def test_pixtral_large():
    """
    Offline vLLM inference for mistralai/Pixtral-Large-Instruct-2411 running on trn2
    """
    tutorial_url = "https://awsdocs-neuron.readthedocs-hosted.com/en/latest/libraries/nxd-inference/tutorials/pixtral-tutorial.html"

    logger.info(f"Starting Pixtral test - follow setup guide at {tutorial_url}")
    logger.info(f"Testing config {PIXTRAL_CONFIG_TRN2}")

    try:
        vllm_integ_test_pixtral(**PIXTRAL_CONFIG_TRN2)
        logger.info("Pixtral large offline integ test passed!")
    except Exception as e:
        logger.error(
            f"Pixtral test failed - ensure proper setup following: {tutorial_url}"
        )
        raise e
...
def test_llama4():
    ...
    LLAMA4_CONFIG_TRN2.update(
        {"title": "llama4-scout", "model_name_or_path": model_name}
    )
...
def test_min_tokens_trumps_eos():
    ...
    for output in outputs:
        ...
        assert len(token_ids) >= min_tokens, (
            "Generated response is shorter than min_tokens"
        )
        assert len(token_ids) <= max_tokens, (
            "Generated response is longer than max_tokens"
        )
        assert 2 in token_ids, (
            "Generated response for this prompt is known to include the eos_token [2]"
        )
...
def test_high_token_budget():
    ...
    sampling_params = SamplingParams(
        top_k=10, temperature=0.8, top_p=0.95, max_tokens=32, min_tokens=32
    )
...
def test_double_on_device_sampling_config():
    ...
    try:
        ...
        os.environ["NEURON_COMPILED_ARTIFACTS"] = (
            "/home/ubuntu/on_device_edge_case_compiled"
        )
        llm = LLM(
            model="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
            max_num_seqs=1,
