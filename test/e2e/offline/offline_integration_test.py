...
def vllm_integ_test_pixtral(
    ...
    llm = _init_vllm(
        ...
        config_format="hf",  # Force HF path to ignore params.json (Mistral) for Pixtral tests
    )

    batched_inputs = []
    batched_sample_params = []
    for pmpt, img, params in zip(PROMPTS, IMAGES, SAMPLING_PARAMS):
        inputs, sampling_params = get_vllm_mm_model_inputs(
            pmpt, img, params, model_name="pixtral"
        )
        # test batch-size = 1
...
def vllm_integ_test_llama4(
    ...
    for pmpt, img, params in zip(PROMPTS, IMAGES, SAMPLING_PARAMS):
        inputs, sampling_params = get_vllm_mm_model_inputs(
            pmpt, img, params, model_name="llama4"
        )
        # test batch-size = 1
        outputs = llm.generate(inputs, sampling_params)
        print_outputs(outputs)
...
def vllm_integ_test(
    ...
    if top_k == 1:
        ...
        expected_outputs = {
            "Hello, my name is": " and I'm writing you today to learn more about the 201",
            "The president of the United States is": " a man who has been accused of sexual assault by multiple women. He has been",
            "The capital of France is": " Paris. It is the most populous city in France and in Europe. It",
