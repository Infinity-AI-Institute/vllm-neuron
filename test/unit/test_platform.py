...
# ============================================================================
# Tests for round-robin DP load balancing patch
# ============================================================================


class TestRoundRobinDPLoadBalancing:
    """Tests for the round-robin DP load balancing patch in _apply_engine_client_override."""

    def _reset_override_state(self):
        NeuronPlatform._engine_client_override_applied = False
        NeuronPlatform._rr_patched = False

    def _make_mock_vllm_config(self, dp_size=1, dp_rank=0, external_lb=False):
        cfg = Mock()
    ...
    def test_rr_patched_initialized_false_after_override(self):
        """_rr_patched is set to False when the override is first applied."""
        self._reset_override_state()

        mock_async_mp = Mock()
        mock_engine_core = Mock()
        mock_engine_core.make_async_mp_client = Mock()

        with patch.dict(
            "sys.modules",
            {
                "vllm.v1.engine.core_client": Mock(
                    AsyncMPClient=mock_async_mp,
                    EngineCoreClient=mock_engine_core,
                ),
            },
        ):
            NeuronPlatform._apply_engine_client_override()

        assert NeuronPlatform._engine_client_override_applied is True
        assert NeuronPlatform._rr_patched is False

    def test_round_robin_applied_on_dplb_client(self):
        """Round-robin patch is applied when make_async_mp_client returns a DPLBAsyncMPClient."""
        self._reset_override_state()
        ...
        mock_dplb_cls = type(
            ...
            (),
            {
                "get_core_engine_for_request": lambda self, req: None,
            },
        )
        mock_dplb_instance = mock_dplb_cls()
        mock_dplb_instance.core_engines = [Mock(), Mock(), Mock()]
        mock_dplb_instance.reqs_in_flight = {}

        mock_async_mp = Mock()
        original_make = Mock(return_value=mock_dplb_instance)
        mock_engine_core = Mock()
        mock_engine_core.make_async_mp_client = original_make

        core_client_module = Mock(
            AsyncMPClient=mock_async_mp,
            EngineCoreClient=mock_engine_core,
            DPLBAsyncMPClient=mock_dplb_cls,
        )

        with patch.dict(
            "sys.modules", {"vllm.v1.engine.core_client": core_client_module}
        ):
            NeuronPlatform._apply_engine_client_override()

            # Invoke the patched make_async_mp_client (dp_size=1, no external LB)
            cfg = self._make_mock_vllm_config(dp_size=1, external_lb=False)
            result = mock_engine_core.make_async_mp_client(cfg, None, False)

        assert NeuronPlatform._rr_patched is True
        assert result is mock_dplb_instance

    def test_round_robin_distributes_evenly(self):
        """Round-robin routes requests to engines in order, cycling back."""
        self._reset_override_state()

        engines = [Mock(name=f"engine_{i}") for i in range(3)]
        ...
        mock_dplb_instance = mock_dplb_cls()
        mock_dplb_instance.core_engines = engines
        mock_dplb_instance.reqs_in_flight = {}
        ...
        with patch.dict(
            ...
            # Call the patched factory to trigger round-robin installation
            cfg = self._make_mock_vllm_config(dp_size=1, external_lb=False)
            mock_engine_core.make_async_mp_client(cfg, None, False)

            # Now test the round-robin method directly
            rr_method = mock_dplb_cls.get_core_engine_for_request

            for cycle in range(2):
                for i in range(3):
                    req = Mock()
                    req.data_parallel_rank = None
                    req.request_id = f"req_{cycle}_{i}"
                    chosen = rr_method(mock_dplb_instance, req)
                    assert chosen is engines[i], (
                        f"Cycle {cycle}, index {i}: expected engine_{i}"
                    )
                    assert (
                        mock_dplb_instance.reqs_in_flight[req.request_id] is engines[i]
                    )

    def test_round_robin_respects_data_parallel_rank(self):
        """Requests with data_parallel_rank set use the original routing."""
        self._reset_override_state()

        engines = [Mock(name=f"engine_{i}") for i in range(2)]
        sentinel_engine = Mock(name="sentinel")

        original_get = Mock(return_value=sentinel_engine)
        mock_dplb_cls = type(
            "DPLBAsyncMPClient",
            (),
            {
                "get_core_engine_for_request": original_get,
            },
        )
        mock_dplb_instance = mock_dplb_cls()
        mock_dplb_instance.core_engines = engines
        ...
        with patch.dict(
            ...
            result = rr_method(mock_dplb_instance, req)

            assert result is sentinel_engine

    def test_rr_patch_not_reapplied_on_second_call(self):
        """Once _rr_patched is True, the patch is not applied again."""
        self._reset_override_state()
        ...
        mock_dplb_instance = mock_dplb_cls()
        mock_dplb_instance.core_engines = [Mock()]
        mock_dplb_instance.reqs_in_flight = {}
        ...
        with patch.dict(
            ...
            cfg = self._make_mock_vllm_config(dp_size=1, external_lb=False)
            # First call triggers the patch
            mock_engine_core.make_async_mp_client(cfg, None, False)
            first_method = mock_dplb_cls.get_core_engine_for_request

            assert NeuronPlatform._rr_patched is True

            # Second call should NOT re-patch
            mock_engine_core.make_async_mp_client(cfg, None, False)
            second_method = mock_dplb_cls.get_core_engine_for_request

            assert first_method is second_method

    def test_rr_patch_handles_dplb_import_error(self):
        """If DPLBAsyncMPClient can't be imported, _rr_patched is still set True."""
        self._reset_override_state()

        non_dplb_client = Mock()

        mock_async_mp = Mock()
        original_make = Mock(return_value=non_dplb_client)
        mock_engine_core = Mock()
        mock_engine_core.make_async_mp_client = original_make

        # Module that raises ImportError when DPLBAsyncMPClient is accessed
        core_client_module = Mock(
            AsyncMPClient=mock_async_mp,
            EngineCoreClient=mock_engine_core,
        )
        del core_client_module.DPLBAsyncMPClient  # force AttributeError on access
        ...
        assert result is non_dplb_client
        assert NeuronPlatform._rr_patched is True

    def test_external_lb_bypasses_round_robin(self):
        """External DP LB path returns AsyncMPClient directly, no round-robin involved."""
        self._reset_override_state()

        mock_async_mp_instance = Mock(name="async_mp_instance")
        mock_async_mp = Mock(return_value=mock_async_mp_instance)
        mock_engine_core = Mock()
        mock_engine_core.make_async_mp_client = Mock()
        ...
        with patch.dict(
            ...
            cfg = self._make_mock_vllm_config(dp_size=2, external_lb=True)
            result = mock_engine_core.make_async_mp_client(cfg, None, False)

        assert result is mock_async_mp_instance
        # _rr_patched should still be False since we took the external LB path
        assert NeuronPlatform._rr_patched is False
