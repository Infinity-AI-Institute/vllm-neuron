...
def test_neuron_pixtral_raises_error_when_batch_size_exceeds_limit(mocker):
    """Test that Pixtral model raises error when batch size > 4.

    This test verifies:
    1. ValueError is raised when tkg_batch_size > 4 for Pixtral models
    2. Error message correctly describes the batch size limitation

    Pixtral models have a known limitation where batch size > 4 can cause
    out-of-bounds errors on Neuron hardware.
    """
    ...
    mock_text_neuron_config.batch_size = 8  # Exceeds limit of 4
    mock_text_neuron_config.tkg_batch_size = 8  # Exceeds limit of 4
    ...
    kwargs = {"neuron_config": {"batch_size": 8}, "override_neuron_config": {}}

    with pytest.raises(
        ValueError,
        match="Neuron Pixtral model does not support batch size > 4 in vLLM v1 yet.",
    ):
        model.load_weights("model_path", "LlavaForConditionalGeneration", **kwargs)
...
def test_neuron_causal_lm_load_weights_local_path_exists(mocker):
    ...
    mocker.patch.object(
        model, "_load_weights_common", return_value=(False, "/compiled", Mock())
    )
...
def test_init_fused_spec_config_non_eagle_zeros_speculation_length():
    ...
    config = Mock()
    config.neuron_config = Mock(enable_eagle_speculation=False, speculation_length=5)
...
def test_performance_logging_forward_method(mocker):
    """Test that performance logging uses % formatting in forward method.
    ...
    4. Batch size context is included
    """
    config = PretrainedConfig(vocab_size=1000)
    model = NeuronCausalLM(config)
    model.model = Mock()
    ...
    with patch('vllm_neuron.worker.neuronx_distributed_model_loader.logger'
        ...
        model.forward(input_ids=torch.tensor([[1, 2], [3, 4]]),
                      positions=torch.tensor([[0, 1], [0, 1]]),
                      input_block_ids=torch.tensor([0, 1]),
                      block_tables=torch.tensor([[0, 1], [2, 3]]),
                      sampling_params=torch.tensor([[1.0], [1.0]]))
        ...
        # Should have performance logs for model execution, output processing, restore, and total
        assert len(debug_calls) >= 3

        # Verify % formatting is used
        for call in debug_calls:
            args = call[0]
            assert '%' in args[0] or 'ms' in str(
                args), "Performance log should use % formatting"
