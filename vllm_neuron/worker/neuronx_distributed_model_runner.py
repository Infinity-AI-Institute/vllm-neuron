...
class NeuronxDistributedModelRunner(LoRAModelRunnerMixin):
    ...
    def execute_model(
        ...
        # Execute model forward pass (no sampling)
    ...
    def _generate_model_runner_output(
        self, sampler_outputs: SamplerOutput | None
    ) -> ModelRunnerOutput:
    ...
    def _process_multi_modal_data_neuron(
        ...
        assert len(mm_data) <= 1, (
            "Processing multiple MultiModalFeatureSpec within one request is not yet supported"
        )
        ...
        if isinstance(mm_data["pixel_values"], torch.Tensor):
            ...
            for pixel_values in mm_data["pixel_values"]:
                padded_pixel_value, _ = pad_tensor(
                    pixel_values, [pixel_values.shape[0], max_height, max_width], 0
                )
    ...
    def _dummy_run(self, num_tokens: int, uniform_decode: bool = False) -> None:
        ...
        try:
            ...
            logger.warning(
                "Dummy run failed: %s. This may be expected during initialization.",
                str(e),
            )
