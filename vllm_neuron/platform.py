    ...
    def _apply_engine_client_override(cls) -> None:
        ...
        try:
            ...
            def _patched_make_async_mp_client(
                ...
                # Apply round-robin patch lazily on first DPLBAsyncMPClient
                # creation. This avoids importing core_client in engine
                # worker processes where it causes startup hangs.
                if not cls._rr_patched:
                    try:
                        from vllm.v1.engine.core_client import DPLBAsyncMPClient

                        if isinstance(client, DPLBAsyncMPClient):
                            _rr_counter = [0]
                            _orig_get = DPLBAsyncMPClient.get_core_engine_for_request

                            def _round_robin(self, request):
                                if (
                                    getattr(request, "data_parallel_rank", None)
                                    is not None
                                ):
                                    return _orig_get(self, request)
                                idx = _rr_counter[0] % len(self.core_engines)
                                _rr_counter[0] += 1
                                chosen = self.core_engines[idx]
                                self.reqs_in_flight[request.request_id] = chosen
                                return chosen

                            DPLBAsyncMPClient.get_core_engine_for_request = _round_robin
                            logger.info(
                                "Neuron override: Using round-robin DP load balancing"
                            )
                    except ImportError:
                        pass
                    cls._rr_patched = True
                return client
            ...
            # Patch DPLBAsyncMPClient to use round-robin routing.
            # Workaround for vLLM v0.16.0 coordinator stats propagation
            # bug that causes the score-based LB to route unevenly.
            # Applied lazily via make_async_mp_client to avoid importing
            # core_client in engine worker processes.
            cls._rr_patched = False
