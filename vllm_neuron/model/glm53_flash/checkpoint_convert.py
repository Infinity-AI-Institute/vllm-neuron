    ...
    *,
    ...
    for layer_idx in range(num_layers):
        ...
        if attn_kind == "linear_attention":
            _convert_kda_layer(state_dict, converted, base, out, dtype, block_size)
...
def _convert_kda_layer(
    ...
    for hf_suffix, dest in simple.items():
        tensor = _dequant_or_cast(state_dict, f"{attn}{hf_suffix}", block_size, dtype)
...
def _convert_dsa_layer(
    ...
    *,
    ...
    kv_b = _dequant_or_cast(state_dict, f"{attn}kv_b_proj.weight", block_size, dtype)
    if kv_b is not None:
...
def _convert_dense_mlp_layer(
    ...
    for name in ("gate_proj", "up_proj", "down_proj"):
        tensor = _dequant_or_cast(state_dict, f"{mlp}{name}.weight", block_size, dtype)
        if tensor is not None:
...
def _convert_moe_layer(
    ...
    *,
    ...
    if corr_key in state_dict:
        ...
        converted[f"{target}e_score_correction_bias"] = state_dict[corr_key].to(
            torch.float32
        )
