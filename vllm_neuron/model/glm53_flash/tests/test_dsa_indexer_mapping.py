...
class _HFReferenceIndexer(nn.Module):
    ...
    def forward(
        ...
        logits = grouped_gate.float() + self.index_kpool_compress_ape.float()[None, None]
...
def _build_wrapper_block(*, hidden_size, n_heads, head_dim, q_lora_rank,
                         index_kpool, index_topk, always_select_tail,
                         num_attention_heads, qk_head_dim, dtype):
    ...
    class _StubCPL(nn.Linear):
        ...
        def __init__(self, in_features, out_features, *, bias=False,
                     gather_output=True, dtype=torch.float32, **kwargs):
            super().__init__(in_features, out_features, bias=bias, dtype=dtype)

    class _StubRPL(nn.Linear):
        def __init__(self, in_features, out_features, *, bias=False,
                     input_is_parallel=True, dtype=torch.float32, **kwargs):
            super().__init__(in_features, out_features, bias=bias, dtype=dtype)

    class _StubEmbed(nn.Embedding):
        def __init__(self, num_embeddings, embedding_dim, padding_idx,
                     *, dtype=torch.float32, shard_across_embedding=False,
                     pad=False, sequence_parallel_enabled=False, **kwargs):
    ...
    fake_mappings = types.ModuleType(
    ...
    fake_config_mod = types.ModuleType(
    ...
    for name, mod in (
        ...
        ("neuronx_distributed.modules", types.ModuleType("neuronx_distributed.modules")),
...
def _copy_hf_weights_to_wrapper(*, hf_block, wrapper_block):
    ...
    with torch.no_grad():
        ...
        wrapper_block.index_kpool_compress_ape.copy_(
            hf_block.index_kpool_compress_ape
        )
...
def test_wrapper_index_scores_match_hf_within_bf16_tolerance(seed: int) -> None:
    ...
    position_ids = (
        torch.arange(seq_len, dtype=torch.int64)[None, :].expand(batch, -1)
    )
    ...
    require_comparable(
        wrapper_scores.detach().cpu().numpy(), "wrapper_index_scores"
    )
    ...
    err = (
        (hf_scores - wrapper_scores).masked_fill(~finite, 0.0).abs().max().item()
    )
