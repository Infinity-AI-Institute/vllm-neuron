...
class Glm53DsaAttention(nn.Module):
    ...
    def forward(
        ...
        # Dispatch through the DSA slug resolver: env `DSA_KERNEL_IMPL`
        # picks between the v0 CPU golden (default) and the v2 NKI wrapper.
        # Both callables have the same signature; on CPU hosts the v2
        # wrapper's own fallback lands on v0 -- numerics are bit-identical
        # and existing tests do not break. The emitted slug reflects the
        # env choice for the compile driver's NEFF-cache-key.
        emitted_slug, dsa_forward = get_dsa_sparse_attention_forward(
            golden.dsa_sparse_attention_forward
        )
        self._last_emitted_dsa_slug = emitted_slug
        attended = dsa_forward(
            projection.query,
            self._key_cache,
            self._value_cache,
            topk,
...
def emitted_dsa_kernel_slug() -> str:
    """The DSA slug this process would advertise in the NEFF cache key.

    Reads ``DSA_KERNEL_IMPL`` and returns the matching slug. See
    :mod:`.kernel_dispatch` for the accepted alias set.
    """
    return resolve_dsa_impl_slug()
