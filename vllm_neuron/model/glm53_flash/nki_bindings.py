...
def dsa_scores_from_qidx(
    ...
    *,
    ...
    d_idx = q_idx.shape[-1]
    if index_pool > 1:
        k_pooled = golden._apply_index_pool(indexer_k_cache, index_pool, pool_weights)
    else:
...
def dsa_attend_from_scores(
    ...
    *,
    ...
    _, indices = torch.topk(masked, k=effective_topk, dim=-1, largest=True, sorted=True)
...
def dsa_traceability_parity_check(*, seed: int = 3) -> dict[str, float]:
    ...
    position_ids = torch.arange(q_len).unsqueeze(0).expand(batch, -1) + (ctx - q_len)
    indices = (
...
def glm53_route(
    ...
    *,
    ...
    logits = F.linear(hidden_states.to(torch.float32), router_weight.to(torch.float32))
...
def moe_gather_dispatch_torch(
    ...
    *,
    ...
    u_sel = up.index_select(0, idx.reshape(-1)).view(batch * length, top_k, hidden, -1)
